        ...
        it('should add - to space label at parentOptions state if it is a child and -- if grandchild', (done) => {
          ...
          wrapper.instance().handleClickNextOrBack().then(() => {
            expect(wrapper.update().state('parentOptions')).to.deep.equal(
              [
                { value: 'None', label: 'None', spaceId: null, parentId: null },
                { value: 'a', label: <span title='a'>{''} <i className='fa-fw fas fa-user-secret' /> {'a'}</span>, spaceId: 1, parentId: null },
                { value: 'b', label: <span title='b'>{'-'} <i className='fa-fw fas fa-user-secret' /> {'b'}</span>, spaceId: 2, parentId: 1 },
                { value: 'c', label: <span title='c'>{'--'} <i className='fa-fw fas fa-user-secret' /> {'c'}</span>, spaceId: 3, parentId: 2 }
              ]
            )
          }).then(done, done)
        })
