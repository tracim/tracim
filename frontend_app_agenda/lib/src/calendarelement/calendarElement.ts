...
export class CalendarElement {
    ...
    private fetchAndLoadEvents = async ({ startStr: start, endStr: end }: EventCalendar.FetchInfo): Promise<EventCalendar.EventInput[]> => {
        const calendarObjects = await this._client.fetchAndLoadEvents(start, end)
    ...
    private onSelectTimeRange = ({ start, end, allDay, jsEvent }: EventCalendar.SelectInfo) => {
        ...
        this._eventHandlers!.onCreateEvent(jsEvent, this._client.getCalendars(), { calendarUrl: "", event: newEvent }, this.handleCreateEvent)
    }
    ...
    private onEventClicked = ({ event: e, jsEvent }: EventCalendar.EventClickInfo) => {
        ...
        const eventData = this._client.getEventDataByUid(uid)
        if (!eventData) return
        this._eventHandlers!.onUpdateEvent(jsEvent, this._client.getCalendars(), { calendarUrl: eventData.calendar.url, event: eventData.event }, this.handleUpdateEvent, this.handleDeleteEvent)
    }
