...
- build tracim docker image `DOCKER_TRACIM_IMAGE` set in
  `test_config.env` and [build part of tracim docker doc](/docs.legacy/administration/installation/tracim_with_docker.md), default
...
- choose elasticsearch docker image `DOCKER_ELASTICSEARCH_IMAGE` set in`test_config.env`   (we do recommend you to build and use
  `elasticsearch_ingest` docker, see [this doc](/docs.legacy/administration/installation/elasticsearch_docker_image_with_ingest_plugin.md))
