...
if [ ! -f /etc/tracim/development.ini ]; then
    ...
    sed -i "s|^basic_setup.website_base_url = .*|basic_setup.website_base_url = http://${tracim_http}|g" /etc/tracim/development.ini
    ...
    sed -i "s|^; webdav.base_url = .*|webdav.base_url = http://${tracim_http}|g" /etc/tracim/development.ini
...
echo "* ${tracim_http}" > /etc/pushpin/routes
