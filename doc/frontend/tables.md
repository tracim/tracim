...
- `className`: The custom classes applied to its cells and header. It's value should be the `settings.className` argument.
...
It is required to set `TitleListHeader` as demonstrated, otherwise it will not work properly. The only things you should change are the `SORT_BY` fields and the customClass prop.
...
To make this column filterable (which is optional), define a filter function.
