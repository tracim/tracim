...
from tracim_backend import logger
...
class HookImpl:
    ...
    def on_user_created(self, user: User, context: TracimContext) -> None:
        ...
        for workspace in open_workspaces:
            rapi.create_one(user=user, workspace=workspace,
                            role_level=workspace.default_user_role.level, with_notif=True, flush=False)
        ...
        if workspace.access_type == WorkspaceAccessType.OPEN:
            ...
            for user in all_users:
                if user != workspace.owner:
                    rapi.create_one(user=user, workspace=workspace, role_level=workspace.default_user_role.level, with_notif=True, flush=False)
