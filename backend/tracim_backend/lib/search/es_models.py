...
INDEX_DOCUMENTS_PATTERN = INDEX_DOCUMENTS_PATTERN_TEMPLATE.format(date="*", index_alias=INDEX_DOCUMENTS_ALIAS)
...
class IndexedContent(Document):
    ...
    status = Keyword()
    is_archived = Boolean()
    is_archived_by_parent_id = Integer()
    is_deleted = Boolean()
    is_deleted_by_parent_id = Integer()
    is_editable = Boolean()
    is_active = Boolean()
    show_in_ui = Boolean()
