...
class MailSenderDaemon(FakeDaemon):
    ...
    def stop(self) -> None:
        ...
        queue = get_rq_queue(redis_connection, self.config.EMAIL__NOTIFICATION__ASYNC_QUEUE_NAME)
        queue.enqueue(do_nothing)

    def run(self) -> None:

        with RQConnection(get_redis_connection(self.config)):
            self.worker = RQWorker([self.config.EMAIL__NOTIFICATION__ASYNC_QUEUE_NAME])
