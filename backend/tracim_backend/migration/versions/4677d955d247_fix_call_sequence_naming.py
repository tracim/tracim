...
def upgrade():
    if op.get_context().dialect.name == "postgresql":
        op.execute(
            "ALTER SEQUENCE IF EXISTS user_calls_call_id_seq RENAME TO seq__usercall__id;"
        )
...
def downgrade():
    if op.get_context().dialect.name == "postgresql":
        op.execute(
            "ALTER SEQUENCE IF EXISTS seq__usercall__id RENAME TO user_calls_call_id_seq;"
        )
