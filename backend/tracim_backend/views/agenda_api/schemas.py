...
class AgendaSchema(marshmallow.Schema):
    ...
    agenda_type = StrippedString(
        validate=agenda_type_validator,
        example=AgendaType.workspace.value
    )
...
class AgendaFilterQuerySchema(marshmallow.Schema):
    ...
    agenda_types = StrippedString(
        ...
        descriptions="comma separated list of types of agenda, can contain any value in {}".format([agenda_type.value for agenda_type in AgendaType]),
    )
