...
def put_document(doc: Content, pushpin_base_url: str) -> int:
...
def messages_stream_client(
    ...
    response = requests.get(
        "{}/api/users/{}/live_messages{}".format(base_url, user_id, query),
        auth=("admin@admin.admin", "admin@admin.admin"),
        stream=True,
        headers=headers,
    )
...
class TestLiveMessages(object):
    ...
    def test_api__user_live_messages_endpoint_with_GRIP_proxy__ok__user_update(
        self, pushpin, app_config, pushpin_base_url,
    ):
        params = {"public_name": "updated", "timezone": "Europe/London", "lang": "en"}
        with messages_stream_client(base_url=pushpin_base_url) as client_events:
            update_user_request = requests.put(
    ...
    def test_api__user_live_messages_endpoint_with_GRIP_proxy__ok__big_content_update__async(
        ...
        with messages_stream_client(base_url=pushpin_base_url) as client_events:
            status = put_document(big_html_document, pushpin_base_url)
            assert status == 204
            event = next(client_events)
        result = json.loads(event.data)
    ...
    def test_api__user_live_messages_endpoint_with_GRIP_proxy__ok__after_event_id(
        ...
        with messages_stream_client(base_url=pushpin_base_url) as client_events:
            put_document(small_html_document_a, pushpin_base_url)
            put_document(small_html_document_b, pushpin_base_url)
        ...
        with messages_stream_client(base_url=pushpin, after_event_id=result1["event_id"]) as client_events:
            event2 = next(client_events)
            assert json.loads(event2.data)["fields"]["content"]["label"] == "Small document B"
            put_document(small_html_document_c, pushpin_base_url)
