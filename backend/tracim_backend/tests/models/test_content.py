...
class TestContent(StandardTest):
    ...
    def test_query(self):
        content1 = self.test_create()
        with new_revision(session=self.session, tm=transaction.manager, content=content1):
        ...
        workspace1 = (
            self.session.query(Workspace).filter(Workspace.label == "TEST_WORKSPACE_1").one()
        )
        workspace2 = (
            self.session.query(Workspace).filter(Workspace.label == "TEST_WORKSPACE_2").one()
        )
        ...
        join_sub_query = (
            self.session.query(ContentRevisionRO.revision_id)
    ...
    def test_update(self):
        ...
        eq_(
            1,
            self.session.query(ContentRevisionRO)
            .filter(ContentRevisionRO.label == "TEST_CONTENT_1_UPDATED_2")
            .count(),
        )
        eq_(1, self.session.query(Content).filter(Content.id == created_content.id).count())
    ...
    def test_creates(self):
        ...
        eq_(
            1,
            self.session.query(ContentRevisionRO)
            .filter(ContentRevisionRO.label == "TEST_CONTENT_2")
            .count(),
        )
    ...
    def test_create(self, key="1"):
        eq_(
            0,
            self.session.query(ContentRevisionRO)
            .filter(ContentRevisionRO.label == "TEST_CONTENT_%s" % key)
            .count(),
        )
        eq_(
            0,
            self.session.query(Workspace)
            .filter(Workspace.label == "TEST_WORKSPACE_%s" % key)
            .count(),
        )
    ...
    def test_unit_get_children(self):
        ...
        with new_revision(session=self.session, tm=transaction.manager, content=children):
