...
class HTMLDocumentController(Controller):
    ...
    def preview_pdf_full(self, context, request: TracimRequest, hapic_data=None):
        ...
        content = api.get_one(hapic_data.path.content_id, content_type=content_type_list.Any_SLUG)
        revision_in_context = api.get_revision_in_context(content.revision)
        ...
        current_date = format_date(datetime.now(), locale=request.current_user.lang)
        revision_txt = _("version {} (revision {}) on {}").format(
            revision_in_context.version_number,
            revision_in_context.revision_id,
            format_date(revision_in_context.created, locale=request.current_user.lang),
        )
        content_url = "{}/ui/workspaces/{}/contents/html-document/{}".format(
            app_config.WEBSITE__BASE_URL, content.workspace_id, content.content_id
        )
        result = api.get_full_pdf_preview_from_html_raw_content(
            ...
            metadata={
                ...
                "date": format_date(content.updated, locale=request.current_user.lang),
                "content_url": content_url,
                "revision_txt": revision_txt,
                "footer_text": _("Generated by Tracim, on {}".format(current_date)),
                "toctitle": _("Summary"),
                "website_url": app_config.WEBSITE__BASE_URL,
                "logo_url": app_config.WEBSITE__BASE_URL
