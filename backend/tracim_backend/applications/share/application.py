...
class ShareApp(TracimApplication):
    ...
    def load_config(self, app_config: CFG) -> None:
        ...
        app_config.EMAIL__NOTIFICATION__SHARE_CONTENT_TO_RECEIVER__TEMPLATE__HTML = (
            app_config.get_raw_config(
                "email.notification.share_content_to_receiver.template.html",
                "{}/{}".format(
                    template_dir, "shared_content_to_receiver_body_html.mak"
                ),
            )
        )
        app_config.EMAIL__NOTIFICATION__SHARE_CONTENT_TO_RECEIVER__SUBJECT = app_config.get_raw_config(
            ...
            _(
                '[{website_title}] {emitter_name} shared the file "{content_filename}" with you'
            ),
        )
        app_config.EMAIL__NOTIFICATION__SHARE_CONTENT_TO_EMITTER__TEMPLATE__HTML = (
            app_config.get_raw_config(
        ...
        app_config.EMAIL__NOTIFICATION__SHARE_CONTENT_TO_EMITTER__SUBJECT = app_config.get_raw_config(
            ...
            _(
                '[{website_title}] You shared "{content_filename}" with {nb_receivers} people'
            ),
        )
