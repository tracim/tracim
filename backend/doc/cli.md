...
Another example with username-only user (need email.required set to false) and no password setted:

```
  tracimcli user create -u "john" --profile "trusted-users" --lang "fr" --public-name "John Doe"
```
...
:warning: if you don't provide password and use --send-email, an autogenerated password will be created and send to user by email, example:
