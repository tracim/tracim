...
export class GroupedNotificationItem extends React.Component {
  getNotificationDetails = notification => {
    ...
    if (notification.author) {
      ...
      if (notification.author.length >= 2) {
        escapedAuthor = `${escapeHtml(notification.author[0].publicName)} ${
          t('and {{numberOfAuthors}} other people', { numberOfAuthors: notification.author.length - 1 })
        }`
      }
    }
  ...
  render () {
    ...
    const authorQuantity = notification.author.length
          ...
          {authorQuantity <= 2
            ? (
              <div className='notification__list__item__text__avatars'>
                <Avatar
                  size={authorQuantity === 2 ? AVATAR_SIZE.MINI : AVATAR_SIZE.SMALL}
