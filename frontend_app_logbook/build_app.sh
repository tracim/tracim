...
if [ "$only_utils" != "--only-utils" ]; then
    log "building frontend_app_logbook"
    yarn run build:optimized$dev  && loggood "success" || logerror "some error"
    log "copying built logbook to frontend/"
    cp dist/logbook.app.optimized.js ../frontend/dist/app/logbook.app.optimized.js && loggood "success" || logerror "some error"
fi

for lang in $(ls i18next.scanner); do
    log "copying ${lang}/translation.json"
    cp i18next.scanner/"${lang}"/translation.json ../frontend/dist/app/logbook_"${lang}"_translation.json && loggood "success" || logerror "some error"
done
