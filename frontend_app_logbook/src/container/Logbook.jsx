...
// TODO - S.G. - 2021-11-24 - The logbook app uses file storage in backend
...
export class Logbook extends React.Component {
  constructor (props) {
    ...
    props.registerLiveMessageHandlerList([
      { entityType: TLM_ET.CONTENT, coreEntityType: TLM_CET.MODIFIED, optionalSubType: TLM_ST.LOGBOOK, handler: this.handleContentChanged },
      { entityType: TLM_ET.CONTENT, coreEntityType: TLM_CET.DELETED, optionalSubType: TLM_ST.LOGBOOK, handler: this.handleContentChanged },
      { entityType: TLM_ET.CONTENT, coreEntityType: TLM_CET.UNDELETED, optionalSubType: TLM_ST.LOGBOOK, handler: this.handleContentChanged },
  ...
  handleShowApp = data => {
    ...
    console.log('%c<Logbook> Custom event', 'color: #28a745', CUSTOM_EVENT.SHOW_APP(state.config.slug), data)
    props.appContentCustomEventHandlerShowApp(data.content, state.content, this.setState.bind(this), this.buildBreadcrumbs)
    if (data.content.content_id === state.content.content_id) this.setHeadTitle(state.content.label)
  }

  handleHideApp = data => {
    console.log('%c<Logbook> Custom event', 'color: #28a745', CUSTOM_EVENT.HIDE_APP(this.state.config.slug), data)
  ...
  handleReloadAppFeatureData = () => {
    console.log('%c<Logbook> Custom event', 'color: #28a745', CUSTOM_EVENT.RELOAD_APP_FEATURE_DATA(this.state.config.slug))
  ...
  handleAllAppChangeLanguage = data => {
    const { props } = this
    console.log('%c<Logbook> Custom event', 'color: #28a745', CUSTOM_EVENT.ALL_APP_CHANGE_LANGUAGE, data)
  ...
  componentDidMount () {
    console.log('%c<Logbook> did Mount', `color: ${this.state.config.hexcolor}`)
  ...
  componentDidUpdate (prevProps, prevState) {
    const { state } = this
    console.log('%c<Logbook> did Update', `color: ${state.config.hexcolor}`, prevState, state)
  ...
  buildBreadcrumbs = async content => {
    try {
      ...
      console.error('Error in app logbook, count not build breadcrumbs', e)
    }
  }
...
export default translate()(appContentFactory(TracimComponent(Logbook)))
