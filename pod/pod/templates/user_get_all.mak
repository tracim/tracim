...
<div class="container-fluid">
    <div class="row-fluid">
        ...
        <div>
            <div class="row">
                <div class="col-sm-11">
                    <h1>${POD.ICO(32, 'apps/system-users')} ${_('Users')}</h1>
                </div>
            </div>
            ...
            <div class="row">
                <div  class="col-sm-11">
                    % if result.user_nb<=0:
                        ${POD.NO_CONTENT_INFO(_('There are no workspace yet. Start by <a class="alert-link" data-toggle="collapse" data-target="#create-workspace-form">creating a workspace</a>.'))}
                    % else:
                        <table class="table table-striped table-hover">
                            ...
                            % for user in result.users:
                                <tr>
                                    % if user.enabled:
                                        <td title="${_('User enabled. Click to disable this user')}"><a href="${tg.url('/admin/users/{}/disable'.format(user.id))}">${POD.ICO(16, 'status/item-enabled')}</a></td>
                                        ...
                                        <td>
                                            ...
                                            <a href="${tg.url('/admin/users/{}/profile/switch?new_role={}'.format(user.id, linked_profile))}">${POD.ICO(16, icon)}</a>
                                        </td>
                                        <td>
                                    ...
                                    % else:
                                        <td title="${_('User disabled. Click to enable this user')}"><a href="${tg.url('/admin/users/{}/enable'.format(user.id))}">${POD.ICO(16, 'status/item-disabled')}</a></td>
                                        <td>${user.name}</td>
                                        <td>${user.email}</td>
                                        <td>
