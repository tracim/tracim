...
<%def name="BREADCRUMB(dom_id, breadcrumb_items)">
    <ul id="${dom_id}" class="breadcrumb" style="margin-top: -1.5em; display: none;">
        % for item in breadcrumb_items:
            % if item.is_active:
                <li class="active">${POD.ICO(16, item.icon)} ${item.label}</li>
            % else:
                <li>${POD.ICO(16, item.icon)} <a href="${item.url}">${item.label}</a></li>
            % endif
        % endfor
    </ul>
</%def>
...
<%def name="SECURED_SECTION_TITLE(user, workspace, dom_id, label, action_dom_id='', action_label='', icon_size='', icon_path='')">
    <h4 id="${dom_id}">
        ${POD.ICO(icon_size, icon_path) if icon_path else ''}
        ${label}
    ...
    % else:
        <table id="${dom_id}" class="table table-striped table-hover">
            % for folder in folders:
                <tr>
                    <td><a href="${tg.url('/workspaces/{}/folders/{}'.format(workspace_id, folder.id))}">${POD.ICO(16, 'places/jstree-folder')} ${folder.label}</a></td>
                    <td>
...
<%def name="PAGE_LIST(dom_id, workspace_id, pages)">
    ...
    % else:
        <table id="${dom_id}" class="table table-striped table-hover">
            % for page in pages:
                <tr>
                    <td><a href="${tg.url('/workspaces/{}/folders/{}/pages/{}'.format(workspace_id, page.folder.id, page.id))}">${POD.ICO(16, 'mimetypes/text-html')} ${page.label}</a></td>
                    <td>
                        ${POD.ICO(16, page.status.icon)} <span class="${page.status.css}">${page.status.label}</span>
                    </td>
                </tr>
            % endfor
        </table>
    ...
    % else:
        <table id="${dom_id}" class="table table-striped table-hover">
            % for curfile in files:
                <tr>
                    <td><a href="${tg.url('/workspaces/{}/folders/{}/files/{}'.format(workspace_id, curfile.folder.id, curfile.id))}">${POD.ICO(16, 'mimetypes/text-html')} ${curfile.label}</a></td>
                    <td>
                        ${POD.ICO(16, curfile.status.icon)} <span class="${curfile.status.css}">${curfile.status.label}</span>
                    </td>
                </tr>
            % endfor
        </table>
    ...
    % else:
        <table id="${dom_id}" class="table table-striped table-hover">
            % for thread in threads:
                <tr>
                    <td><a href="${tg.url('/workspaces/{}/folders/{}/threads/{}'.format(workspace_id, thread.folder.id, thread.id))}">${POD.ICO(16, 'apps/internet-group-chat')} ${thread.label}</a></td>
                    <td>${POD.ICO(16, thread.status.icon)} <span class="${thread.status.css}">${thread.status.label}</span></td>
        ...
        <script>
            $(function () {
                $('#${dom_id}-treeview').jstree({
                    ...
                    "types" : {
                        "default" : {
                            "icon" : "${POD.ICO_URL(16, 'places/jstree-folder')}"
                        },
                        "workspace" : {
                            "icon" : "${POD.ICO_URL(16, 'places/folder-remote')}"
                        },
                    },
                    'core' : {
    ...
    <div class="btn-group">
        % if not allow_to_change_status:
            <button type="button" class="btn btn-default disable btn-link">
                ${POD.ICO(16, item.status.icon)} <span class="${item.status.css}">${item.status.label}</span>
            </button>
        % else:
            ...
            <ul class="dropdown-menu" role="menu">
                % for status in h.AllStatus(item.type):
                    ...
                    <li><a
                        ...
                        href="${target_url.format(status_id=status.id)}"> ${POD.ICO(16, status.icon)} <span class="${status.css}">${status.label}</span></a></li>
                % endfor
            </ul>
        % endif
    </div>
