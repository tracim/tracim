...
<%namespace name="TOOLBAR" file="pod.templates.page_toolbars"/>
...
<%def name="REQUIRED_DIALOGS()">
    ...
    ${POD.MODAL_DIALOG('page-edit-modal-dialog', 'modal-lg')}
</%def>
...
<h1 class="page-header">
    ${POD.ICO(32, result.page.icon)} ${result.page.label}
        ...
        <table class="table table-striped table-hover">
            % for revid, revision in reversed(list(enumerate(reversed(result.page.revisions)))):
                ...
                <tr class="${warning_or_not}">
                    ...
                    <td>${POD.ICO(16, 'mimetypes/text-html')}</td>
                    ...
                    <td>
                        % if warning_or_not:
                            ${POD.ICO(16, 'actions/go-previous')} <strong>${_('Revision r{}').format(result.page.selected_revision)}</strong>
                        % endif
                    </td>
                </tr>
            % endfor
