...
<%def name="SECURED_FOLDER(user, workspace, folder)">
    ...
    % if h.user_role(user, workspace)>2:
        <div class="btn-group btn-group-vertical">
            ...
            <a title="${_('Edit current folder')}" class="btn btn-default ${edit_disabled}" data-toggle="modal" data-target="#folder-edit-modal-dialog" data-remote="${tg.url('/workspaces/{}/folders/{}/edit'.format(folder.workspace.id, folder.id))}" >${POD.ICO(32, 'apps/accessories-text-editor')}</a>
        </div>
        <p></p>
    % endif
    ...
    <div class="btn-group btn-group-vertical">
        % if user.profile.id>=3 or h.user_role(user, workspace)>=4:
            ## This action is allowed for content managers only
            <a title="${_('Move current folder')}" class="btn btn-default ${move_disabled}" data-toggle="modal" data-target="#folder-move-modal-dialog" data-remote="${tg.url('/workspaces/{}/folders/{}/location/{}/edit'.format(folder.workspace.id, folder.id, folder.id))}" >${POD.ICO(32, 'actions/item-move')}</a>
        % endif
    </div>
    <p></p>
