...
<div class="container-fluid">
    <div class="row-fluid">
        ...
        <div>
            <div class="row">
                <div class="col-sm-11">
                    <h1>${POD.ICO(32, 'places/folder-remote')} ${result.workspace.label}</h1>
            ...
            <div class="row">
                <div class="col-sm-11">
                        ...
                        <div id="add-role-from-existing-user-form" class="collapse col-sm-6">
                            <div class="pod-inline-form">
                                <form role="form" method="POST" action="${tg.url('/admin/workspaces/{}/roles'.format(result.workspace.id))}">
                                    ...
                                    <div class="form-group">
                                        <label>${_('Role')} ${POD.HELP_MODAL_DIALOG_BUTTON('user-role-definition', 'margin-left: 0.5em;')}</label>
                    ...
                    % if result.workspace.member_nb<=0:
                        ${POD.NO_CONTENT_INFO(_('There are no user associated to the current workspace. Start by <a class="alert-link" data-toggle="collapse" data-target="#add-role-from-existing-user-form">adding members</a>  to the workspace.'))}
                    % else:
                        <table class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th>${_('User')}</th>
                                    <th>${_('Email')}</th>
                                    <th>${_('Role')} ${POD.HELP_MODAL_DIALOG_BUTTON('user-role-definition', 'margin-left: 0.5em;')}</th>
                                    <th></th>
                                </tr>
                            </thead>
                            % for member in result.workspace.members:
                                <tr>
                                    ...
                                    <td>
                                        <a title="${_('Remove this user from the current workspace')}" class="btn btn-default btn-small" href="${tg.url('/admin/workspaces/{}/roles/{}/delete'.format(result.workspace.id, member.id))}">${POD.ICO(16, 'status/dialog-error')}</a>
                                    </td>
                                </tr>
                            % endfor
                        </table>
...
${POD.HELP_MODAL_DIALOG('user-role-definition')}
