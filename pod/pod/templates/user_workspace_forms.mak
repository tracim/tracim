    ...
    <div id="${dom_id}" class="collapse">
        <div class="pod-inline-form" >
            <form method="POST" action="${tg.url('/workspaces/{}/folders').format(workspace_id)}">
                ...
                <p>
                    <label><input id="content-folders" name="can_contain_folders" type="checkbox"> ${POD.ICO(16, 'places/jstree-folder')} ${_('sub-folders')}</label><br/>
                    <label><input id="content-threads" name="can_contain_threads" type="checkbox"> ${POD.ICO(16, 'apps/internet-group-chat')} ${_('threads')}</label><br/>
                    <label><input id="content-files" name="can_contain_files" type="checkbox"> ${POD.ICO(16, 'mimetypes/text-x-generic-template')} ${_('files')}</label><br/>
                    <label><input id="content-pages" name="can_contain_pages" type="checkbox"> ${POD.ICO(16, 'mimetypes/text-html')} ${_('Wiki pages')}</label>
                    ${POD.HELP_MODAL_DIALOG_BUTTON('content-wiki-page-definition', 'margin-left: 0.5em;')}
                </p>
...
<%def name="EDIT_FOLDER_FORM(dom_id, folder)">
    <form role="form" method="POST" action="${tg.url('/workspaces/{}/folders/{}?_method=PUT').format(folder.workspace.id, folder.id)}">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">${POD.ICO(32, 'apps/internet-group-chat')} ${_('Edit Folder')}</h4>
        </div>
        <div class="modal-body">
            ...
            <div class="checkbox">
                ...
                <label><input name="can_contain_folders" type="checkbox" ${checked}> ${POD.ICO(16, 'places/jstree-folder')} ${_('sub-folders')}</label>
            </div>
            <div class="checkbox">
                <% checked = ('', 'checked')[folder.allowed_content.thread] %>
                <label><input name="can_contain_threads" type="checkbox" ${checked}> ${POD.ICO(16, 'apps/internet-group-chat')} ${_('threads')}</label>
            </div>
            <div class="checkbox">
                <% checked = ('', 'checked')[folder.allowed_content.file] %>
                <label><input name="can_contain_files" type="checkbox" ${checked}> ${POD.ICO(16, 'mimetypes/text-x-generic-template')} ${_('files')}</label>
            </div>
            <div class="checkbox">
                <% checked = ('', 'checked')[folder.allowed_content.page] %>
                <label><input name="can_contain_pages" type="checkbox" ${checked}> ${POD.ICO(16, 'mimetypes/text-html')} ${_('Wiki pages')} </label>
            </div>
        </div>
...
<%def name="ITEM_MOVE_FORM(dom_id, item, do_move_url, modal_title)">
    <form role="form" method="POST" action="${do_move_url}">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">${_('Close')}</span></button>
            <h4 class="modal-title" id="myModalLabel">${POD.ICO(32, 'actions/item-move')} ${modal_title}</h4>
        </div>
        <div class="modal-body">
            <%
...
<%def name="USER_EDIT_FORM(dom_id, user, target_url)">
    <form id="${dom_id}" role="form" method="POST" action="${target_url}">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">${POD.ICO(32, 'actions/contact-new')} ${_('Edit User')}</h4>
        </div>
        <div class="modal-body">
            <div class="form-group">
...
<%def name="USER_PASSWORD_EDIT_FORM(dom_id, user, target_url)">
    <form id='${dom_id}' role="form" method="POST" action="${target_url}">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">${_('Close')}</span></button>
            <h4 class="modal-title">${POD.ICO(32, 'actions/system-lock-screen')} ${_('Change password')}</h4>
        </div>
        <div class="modal-body">
            <div class="form-group">
