...
<h1 class="page-header">${POD.ICO(32, 'places/folder-remote')} ${result.workspace.label}</h1>
...
<p>
    ...
    % else:
        ${POD.ICO(16, 'apps/system-users')} &mdash;
        % for member in result.workspace.members:
            <strong>${member.name}</strong>
            ${POD.ICO_FA_BADGED('fa fa-flag', member.role_description, member.style)}&emsp;
        % endfor
    % endif
</p>
