...
class UserPasswordRestController(PodRestController):
    ...
    def put(self, current_password, new_password1, new_password2):
        ...
        redirect_url = tg.lurl('/users/{}'.format(user.user_id))
        if current_user.user_id==user.user_id:
            redirect_url = tg.lurl('/users/me')
...
class Bob(object):
    ...
    def post_delete(self, workspace_id):
        ...
        undo_url = tg.url('/workspaces/{}/restore'.format(workspace_id))

        api.delete_one(workspace_id, True)
    ...
    def restore(self, workspace_id):
        ...
        workspace_label = workspace.data_label
        undo_url = tg.url('/workspaces/{}/delete'.format(workspace_id))

        tg.flash(_('{} workspace restored.'))
        tg.redirect(tg.url('/workspaces'))
