...
describe('<SelectStatus />', () => {
  ...
  describe('Handlers', () => {
    it(`first submenu_item should call the proper onClick`, () => {
      ...
      expect(onChangeStatusCallBack.called).to.true
      onChangeStatusCallBack.resetHistory()
    })

    it(`second submenu_item should call the proper onClick`, () => {
      wrapper.find(`button.selectStatus__submenu__item`).at(1).simulate('click')
      expect(onChangeStatusCallBack.called).to.true
      onChangeStatusCallBack.resetHistory()
    })
  })
