...
describe('<Checkbox />', () => {
  ...
  describe('Static design', () => {
    ...
    it(`should not diplay the dive .checboxCustom__checked if checked is false`, () =>
      expect(wrapper.find(`div.checkboxCustom__checked`)).to.have.lengthOf(0)
    )

    it(`should diplay the dive .checkboxCustom__checked if checked is true`, () => {
    ...
    it(`its input should have defaultChecked set to ${props.defaultChecked}`, () => {
  ...
  describe('Handlers', () => {
    it('onClickCheckbox handler should call the proper handler', () => {
      ...
      expect(onClickCheckbox.called).to.true
    })
  })
})
